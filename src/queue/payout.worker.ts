...
import { measureDuration } from '../utils/performance';
 ...
 */
async function processDividendPayout(
  job: Job<DividendJobData>
): Promise<DividendJobResult> {
 ...
 */
export function createPayoutWorker(redisConnection: any, io?: any) {
  const worker = new Worker<DividendJobData, DividendJobResult>(
    'dividend-distribution',
    async (job: Job<DividendJobData>) => {
      // Measure performance
      return await measureDuration(
        'dividend-payout',
        () => processDividendPayout(job),
        {
...
if (require.main === module) {
  ...
  logger.info('Starting standalone dividend payout worker...');
