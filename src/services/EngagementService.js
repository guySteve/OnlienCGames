...
class EngagementService {
    ...
    async claimDailyReward(userId) {
        ...
        await this.prisma.$transaction(async (tx) => {
            ...
            await tx.transaction.create({
                data: {
                    userId,
                    amount: reward.chips,
                    type: client_1.TransactionType.DAILY_STREAK,
        ...
        await this.redis.setex(`user:${userId}:streak`, 86400, JSON.stringify({ day: newStreak, nextClaim: nextClaim.toISOString() }));
        ...
        await this.prisma.$transaction(async (tx) => {
            ...
            await tx.transaction.create({
                data: {
                    userId,
                    amount,
                    type: client_1.TransactionType.MYSTERY_DROP,
                    balanceBefore: user.chipBalance,
                    balanceAfter: updated.chipBalance,
     ...
     */
    async recordBigWin(userId, amount, gameType) {
        const TICKER_THRESHOLD = 1000;
        if (amount < TICKER_THRESHOLD)
            return;
        const user = await this.prisma.user.findUnique({ where: { id: userId } });
        if (!user)
            return;
            ...
            await this.prisma.$transaction(async (tx) => {
                ...
                await tx.transaction.create({
                    data: {
                        userId,
                        amount: levelBonus,
                        type: client_1.TransactionType.LEVEL_UP_BONUS,
                        balanceBefore: user.chipBalance,
     ...
     */
    detectNearMiss(gameType, playerValue, dealerValue) {
        if (gameType === 'WAR') {
