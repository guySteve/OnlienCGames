 ...
 */
async function checkRedis(redis, timeout) {
 ...
 * @returns Component health result
 */
async function checkDatabase(prisma, timeout) {
    const startTime = Date.now();
    try {
        const result = await Promise.race([
            (0, DatabaseConfig_1.testDatabaseConnection)(prisma),
...
function checkSocketIO(io) {
    ...
    try {
        const stats = (0, SocketIOAdapter_1.getAdapterStats)(io);
        if (!stats) {
 ...
 * @returns Express request handler
 */
function createReadinessHandler(config) {
    return async (req, res) => {
        try {
            ...
            const isReady = redisOk.status !== HealthStatus.UNHEALTHY &&
