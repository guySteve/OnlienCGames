 ...
 */
interface BlackjackCustomState {
  shoe: Array<{ rank: string; suit: string; value: number }>;
  cutCardPosition: number;
  currentShoePosition: number;
  dealerHand: Array<{ rank: string; suit: string; value: number }>;
  bjPlayers: Record<string, BlackjackPlayerState>; // userId:seatIndex → state
 ...
 */
export class BlackjackEngineV5 extends BaseGameEngine {
  ...
  async playerAction(
    ...
    metadata?: { insuranceAmount?: number }
    ...
    const lockResult = await lockManager.withLock(
      `table:${this.config.tableId}:action`,
      async () => {
        ...
        const customState = await this.loadCustomState<BlackjackCustomState>();
        if (!customState) {
          throw new Error('INVALID_STATE');
        }
        ...
        switch (action) {
          ...
          case 'INSURANCE':
            result = await this.handleInsurance(
              userId,
              seatIndex,
              customState,
              bjPlayer,
              metadata?.insuranceAmount || 0
            );
            break;
      ...
      LOCK_PRESETS.STANDARD // 5-second TTL, 3 retries
    );
   ...
   * Deal card from shoe
   */
  private dealCard(customState: BlackjackCustomState) {
   ...
   */
  private calculateHandValue(cards: Array<{ rank: string; value: number }>): number {
    let total = 0;
    let aces = 0;
   ...
   */
  private async moveToNextPlayer(customState: BlackjackCustomState): Promise<void> {
    ...
    if (customState.currentPlayerIndex >= playerKeys.length) {
      ...
      await this.setState(GameState.DEALER_TURN);
    } else {
