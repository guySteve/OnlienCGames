...
const WarEngine_1 = require("../src/engines/WarEngine");
...
describe('WarEngine v5', () => {
    const createWarEngine = () => {
        return new WarEngine_1.WarEngine('test-room', mockPrisma, mockRedis, mockEngagement);
    };
    ...
    test('should have 25 empty spots on initialization', () => {
        ...
        assert.ok(state.spots.every((s) => s.bet === 0), 'All spots should be empty');
    });
    ...
    describe('Chip Balance Audit', () => {
        test('should track exact chip movement for a single bet', async () => {
            ...
            // Conservation law
            assert.strictEqual(playerAfter.chipBalance + state.pot, initialChips, 'Total chips conserved');
        });
...
runTests();
//# sourceMappingURL=WarEngine.test.js.map